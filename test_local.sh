...
# Script de prueba local para el evaluador de modelos
# Genera datos de prueba y ejecuta evaluaciones de ejemplo
...
# Ejecutar evaluaciones de ejemplo
echo ""
echo "🧪 PRUEBA 1: Evaluación básica local"
echo "======================================"
python evaluate.py \
  --model_path examples/model.pkl \
  --data_path examples/test_data.csv \
...
python evaluate.py \
  ...
  --data_path examples/test_data.csv \
  --target_col temperature \
  --feature_cols "humidity,wind_speed,pressure,altitude,hour,month,cloud_cover" \
  --primary_metric rmse \
  --threshold 3.0 \
  --metrics_local_path examples/metrics_test.json \
  --verbose
